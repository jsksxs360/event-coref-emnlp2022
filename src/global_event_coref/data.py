...
def get_trigger_vector(w2v_model, trigger:str, vec_dim:int):
    words = wordninja.split(trigger.strip())
...
class KBPCorefWithW2V(Dataset):
    def __init__(self, data_file, w2v_model):
        self.w2v = w2v_model
                ...
                events = [
                    {
                        ...
                        'trigger': e['trigger'], 
                        'trigger_vector': get_trigger_vector(self.w2v, e['trigger'], 300), 
...
def get_dataLoader(args, dataset, tokenizer, mention_tokenizer=None, batch_size=None, shuffle=False, collote_fn_type='normal'):
    ...
    def collote_fn_with_w2v(batch_samples):
        batch_sentences, batch_events = [], []
        for sample in batch_samples:
            batch_sentences.append(sample['document'])
            batch_events.append(sample['events'])
        ...
        for b_idx, sentence in enumerate(batch_sentences):
            ...
            filtered_event_w2vs = []
            filtered_event_cluster_id = []
            for event in batch_events[b_idx]:
                ...
                if not token_start or not token_end:
                    continue
                filtered_events.append([token_start, token_end])
                filtered_event_w2vs.append(event['trigger_vector'])
                filtered_event_cluster_id.append(event['cluster_id'])
            batch_filtered_events.append(filtered_events)
            batch_filtered_event_w2vs.append(np.asarray(filtered_event_w2vs))
            batch_filtered_event_cluster_id.append(filtered_event_cluster_id)
        return {
            'batch_inputs': batch_inputs, 
            'batch_events': batch_filtered_events, 
            'batch_event_w2vs': batch_filtered_event_w2vs, 
    ...
    def collote_fn_with_mask_w2v(batch_samples):
        batch_sentences, batch_events = [], []
        batch_mentions, batch_mention_pos = [], []
        for sample in batch_samples:
            batch_sentences.append(sample['document'])
    ...
    def collote_fn_with_mention_mask(batch_samples):
        batch_sentences, batch_events = [], []
        batch_mentions, batch_mention_pos = [], []
        for sample in batch_samples:
            batch_sentences.append(sample['document'])
    ...
    def collote_fn_with_mention_mask_w2v(batch_samples):
        batch_sentences, batch_events = [], []
        batch_mentions, batch_mention_pos = [], []
        for sample in batch_samples:
            batch_sentences.append(sample['document'])
    ...
    elif collote_fn_type == 'with_w2v':
        select_collote_fn = collote_fn_with_w2v
    ...
    elif collote_fn_type == 'with_mask_w2v':
        assert mention_tokenizer is not None
        select_collote_fn = collote_fn_with_mask_w2v
    elif collote_fn_type == 'with_mention_mask':
        assert mention_tokenizer is not None
        select_collote_fn = collote_fn_with_mention_mask
    elif collote_fn_type == 'with_mention_mask_w2v':
        assert mention_tokenizer is not None
        select_collote_fn = collote_fn_with_mention_mask_w2v
