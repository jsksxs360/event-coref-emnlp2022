...
class LongformerSoftmaxForECwithW2V(LongformerPreTrainedModel):
    def __init__(self, config, encoder_config, args):
        super().__init__(config)
        self.num_labels = args.num_labels
        self.w2v_dim = args.w2v_dim
        self.hidden_size = config.hidden_size + args.w2v_dim
        self.longformer = LongformerModel(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.span_extractor = SelfAttentiveSpanExtractor(input_dim=config.hidden_size)
        self.matching_style = args.matching_style
    ...
    def forward(self, batch_inputs, batch_events, batch_event_w2vs, batch_event_cluster_ids=None):
        outputs = self.longformer(**batch_inputs)
        sequence_output = outputs[0]
        sequence_output = self.dropout(sequence_output)
        # construct event pairs (event_1, event_2)
        batch_event_1_list, batch_event_2_list = [], []
        batch_event_1_w2vs, batch_event_2_w2vs = [], []
        batch_event_mask = []
        max_len = 0
        if batch_event_cluster_ids is not None:
            batch_coref_labels = []
            batch_local_event_1_subtypes, batch_local_event_2_subtypes = [], []
            for events, event_w2vs, event_cluster_ids in zip(
                batch_events, batch_event_w2vs, batch_event_cluster_ids
            ):
                event_1_list, event_2_list = [], []
                ...
                batch_event_1_list.append(event_1_list)
                batch_event_2_list.append(event_2_list)
                batch_event_1_w2vs.append(
                    torch.index_select(event_w2vs, 0, torch.tensor(event_1_idx).to(self.use_device))
                )
                batch_event_2_w2vs.append(
                    torch.index_select(event_w2vs, 0, torch.tensor(event_2_idx).to(self.use_device))
                )
                batch_coref_labels.append(coref_labels)
                batch_event_mask.append([1] * len(coref_labels))
            # padding
            for b_idx in range(len(batch_coref_labels)):
                ...
                batch_event_1_list[b_idx] += [[0, 0]] * pad_length
                batch_event_2_list[b_idx] += [[0, 0]] * pad_length
                batch_event_1_w2vs[b_idx] = torch.cat([
                    batch_event_1_w2vs[b_idx], 
                    torch.zeros((pad_length, self.w2v_dim)).to(self.use_device)
                ], dim=0).unsqueeze(0)
                batch_event_2_w2vs[b_idx] = torch.cat([
                    batch_event_2_w2vs[b_idx], 
                    torch.zeros((pad_length, self.w2v_dim)).to(self.use_device)
                ], dim=0).unsqueeze(0)
                batch_coref_labels[b_idx] += [0] * pad_length
                batch_event_mask[b_idx] += [0] * pad_length
        else:
            for events, event_w2vs in zip(batch_events, batch_event_w2vs):
                event_1_list, event_2_list = [], []
                event_1_idx, event_2_idx = [], []
                for i in range(len(events) - 1):
                    for j in range(i + 1, len(events)):
        ...
        batch_event_1 = torch.tensor(batch_event_1_list).to(self.use_device)
        batch_event_2 = torch.tensor(batch_event_2_list).to(self.use_device)
        batch_event_1_w2vs = torch.cat(batch_event_1_w2vs, dim=0)
        batch_event_2_w2vs = torch.cat(batch_event_2_w2vs, dim=0)
        batch_mask = torch.tensor(batch_event_mask).to(self.use_device)
        batch_labels = None
        if batch_event_cluster_ids is not None:
            batch_labels = torch.tensor(batch_coref_labels).to(self.use_device)
        ...
        batch_event_1_reps = self.span_extractor(sequence_output, batch_event_1, span_indices_mask=batch_mask)
        batch_event_2_reps = self.span_extractor(sequence_output, batch_event_2, span_indices_mask=batch_mask)
        batch_event_1_reps = torch.cat([batch_event_1_reps, batch_event_1_w2vs], dim=-1)
        batch_event_2_reps = torch.cat([batch_event_2_reps, batch_event_2_w2vs], dim=-1)
        assert self.matching_style in ['base', 'multi', 'dist', 'multi_dist']
        if self.matching_style == 'base':
            batch_seq_reps = torch.cat([batch_event_1_reps, batch_event_2_reps], dim=-1)
        elif self.matching_style == 'multi':
...
class LongformerSoftmaxForECwithMaskAndW2V(LongformerPreTrainedModel):
    def __init__(self, config, encoder_config, args):
        super().__init__(config)
        self.num_labels = args.num_labels
        self.num_subtypes = args.num_subtypes
        self.w2v_dim = args.w2v_dim
        self.hidden_size = config.hidden_size + args.w2v_dim + encoder_config.hidden_size
        self.longformer = LongformerModel(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.span_extractor = SelfAttentiveSpanExtractor(input_dim=config.hidden_size)
        self.matching_style = args.matching_style
    ...
    def forward(self, 
        ...
        # get local event mask representations
        batch_local_event_mask_reps = []
        for mention_mask_inputs, mention_events in zip(
            batch_mention_inputs_with_mask, batch_mention_events
        ):
            mention_event_list = [[event] for event in mention_events]
            mention_event_list = torch.tensor(mention_event_list).to(self.use_device)
        ...
        if batch_event_cluster_ids is not None:
            batch_coref_labels = []
            batch_local_event_1_subtypes, batch_local_event_2_subtypes = [], []
            for events, event_w2vs, local_event_mask_reps, event_cluster_ids, event_subtypes in zip(
                batch_events, batch_event_w2vs, batch_local_event_mask_reps, batch_event_cluster_ids, batch_event_subtypes
            ):
                event_1_list, event_2_list = [], []
                event_1_idx, event_2_idx = [], []
                coref_labels = []
            ...
            for events, event_w2vs, local_event_mask_reps in zip(
                batch_events, batch_event_w2vs, batch_local_event_mask_reps
            ):
                event_1_list, event_2_list = [], []
                event_1_idx, event_2_idx = [], []
                for i in range(len(events) - 1):
        ...
        batch_event_1_reps = self.span_extractor(sequence_output, batch_event_1, span_indices_mask=batch_mask)
        batch_event_2_reps = self.span_extractor(sequence_output, batch_event_2, span_indices_mask=batch_mask)
        batch_event_1_reps = torch.cat([batch_event_1_reps, batch_event_1_w2vs, batch_local_event_1_mask_reps], dim=-1)
        batch_event_2_reps = torch.cat([batch_event_2_reps, batch_event_2_w2vs, batch_local_event_2_mask_reps], dim=-1)
        assert self.matching_style in ['base', 'multi', 'dist', 'multi_dist']
        if self.matching_style == 'base':
            batch_seq_reps = torch.cat([batch_event_1_reps, batch_event_2_reps], dim=-1)
        elif self.matching_style == 'multi':
...
class LongformerSoftmaxForECwithMentionMaskAndW2V(LongformerPreTrainedModel):
    def __init__(self, config, encoder_config, args):
        super().__init__(config)
        self.num_labels = args.num_labels
        self.num_subtypes = args.num_subtypes
        self.w2v_dim = args.w2v_dim
        self.hidden_size = config.hidden_size + args.w2v_dim + 2 * encoder_config.hidden_size
        self.longformer = LongformerModel(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.span_extractor = SelfAttentiveSpanExtractor(input_dim=config.hidden_size)
        self.matching_style = args.matching_style
    ...
    def forward(self, 
        ...
        if batch_event_cluster_ids is not None:
            batch_coref_labels = []
            batch_local_event_1_subtypes, batch_local_event_2_subtypes = [], []
            for events, event_w2vs, local_event_reps, local_event_mask_reps, event_cluster_ids, event_subtypes in zip(
                batch_events, batch_event_w2vs, batch_local_event_reps, batch_local_event_mask_reps, batch_event_cluster_ids, batch_event_subtypes
            ):
                event_1_list, event_2_list = [], []
                event_1_idx, event_2_idx = [], []
                coref_labels = []
            ...
            for events, event_w2vs, local_event_reps, local_event_mask_reps in zip(
                batch_events, batch_event_w2vs, batch_local_event_reps, batch_local_event_mask_reps
            ):
                event_1_list, event_2_list = [], []
                event_1_idx, event_2_idx = [], []
                for i in range(len(events) - 1):
        ...
        batch_event_1_reps = self.span_extractor(sequence_output, batch_event_1, span_indices_mask=batch_mask)
        batch_event_2_reps = self.span_extractor(sequence_output, batch_event_2, span_indices_mask=batch_mask)
        batch_event_1_reps = torch.cat([batch_event_1_reps, batch_event_1_w2vs, batch_local_event_1_reps, batch_local_event_1_mask_reps], dim=-1)
        batch_event_2_reps = torch.cat([batch_event_2_reps, batch_event_2_w2vs, batch_local_event_2_reps, batch_local_event_2_mask_reps], dim=-1)
        assert self.matching_style in ['base', 'multi', 'dist', 'multi_dist']
        if self.matching_style == 'base':
            batch_seq_reps = torch.cat([batch_event_1_reps, batch_event_2_reps], dim=-1)
        elif self.matching_style == 'multi':
