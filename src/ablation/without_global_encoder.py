...
from src.ablation.modeling import WithoutGlobalEncoder
...
def to_device(args, batch_data):
    ...
    for k, v in batch_data.items():
        if k in ['batch_mention_events', 'batch_event_cluster_ids', 'batch_event_subtypes']:
            new_batch_data[k] = v
...
def train(args, train_dataset, dev_dataset, model, mention_tokenizer):
    """ Train the model """
    train_dataloader = get_dataLoader(args, train_dataset, None, mention_tokenizer, shuffle=True, collote_fn_type='without_global')
    dev_dataloader = get_dataLoader(args, dev_dataset, None, mention_tokenizer, shuffle=False, collote_fn_type='without_global')
...
def predict(args, mentions:list, mention_pos:list, event_dists:list, model, mention_tokenizer):
    assert len(mentions) == len(mention_pos) == len(event_dists)
    new_mentions = []
    mention_events = []
    for mention, mention_pos in zip(mentions, mention_pos):
    ...
    for e_idx, (e_start, e_end) in enumerate(mention_events):
...
def test(args, test_dataset, model, mention_tokenizer, save_weights:list):
    test_dataloader = get_dataLoader(
        args, test_dataset, None, mention_tokenizer, batch_size=1, shuffle=False, 
        collote_fn_type='without_global'
    )
...
if __name__ == '__main__':
    ...
    logger.info(f'loading pretrained model and tokenizer of {args.mention_encoder_type}...')
    ...
    model = WithoutGlobalEncoder.from_pretrained(
    ...
    if args.do_train:
        ...
        save_weights = train(args, train_dataset, dev_dataset, model, mention_tokenizer)
    # Testing
    ...
    if args.do_test:
        ...
        test(args, test_dataset, model, mention_tokenizer, save_weights)
    # Predicting
    if args.do_predict:
        ...
        for best_save_weight in save_weights:
            ...
            with open(os.path.join(args.output_dir, pred_event_file), 'rt' , encoding='utf-8') as f_in:
                for line in tqdm(f_in.readlines()):
                    ...
                    predictions, probabilities = predict(args, mentions, mention_pos, event_dists, model, mention_tokenizer)
                    results.append({
