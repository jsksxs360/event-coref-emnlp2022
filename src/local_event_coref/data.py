...
id2subtype = {idx: c for idx, c in enumerate(CATEGORIES, start=1)}
            ...
            for line in f:
                ...
                events = [
                    (
                        ...
                        subtype2id.get(e['subtype'], 0)
                    )
                    for e in sample['events']
                ]
                for i in range(len(events) - 1):
                    for j in range(i + 1, len(events)):
                        e1_offset, e1_start, e1_end, e1_sen, e1_cluster, e1_subtype = events[i]
                        e2_offset, e2_start, e2_end, e2_sen, e2_cluster, e2_subtype = events[j]
                        Data.append({
                            'id': sample['doc_id'], 
                            'e1_offset': e1_offset, 
                            'e1_sen': e1_sen, 
                            'e1_start': e1_start, 
                            'e1_end': e1_end, 
                            'e1_subtype': e1_subtype, 
                            'e2_offset': e2_offset, 
                            'e2_sen': e2_sen, 
                            'e2_start': e2_start, 
                            'e2_end': e2_end, 
                            'e2_subtype': e2_subtype, 
...
def get_dataLoader(args, dataset, tokenizer, batch_size=None, shuffle=False, collote_fn_type='normal'):
    ...
    assert collote_fn_type in ['normal', 'mask', 'mask_and_subtype']
    ...
    def collote_fn_with_mask_and_subtype(batch_samples):
        ...
        return {
            ...
            'batch_e2_idx': torch.tensor(batch_e2_token_idx), 
            'labels': torch.tensor(batch_label), 
            'subtypes': torch.tensor(batch_subtypes)
        }
    
    if collote_fn_type == 'normal':
        select_collote_fn = collote_fn
    elif collote_fn_type == 'mask':
        select_collote_fn = collote_fn_with_mask
    elif collote_fn_type == 'mask_and_subtype':
        select_collote_fn = collote_fn_with_mask_and_subtype
    return DataLoader(
        dataset, 
